...
# Проблема
Я пересматривал задание и понял, что я ошибся в функции merge для пул реквеста. Я не выводил информацию о времени мерджа, а также операция была не идемпотентной, поэтому я добавил условие в запрос к бд и изменил ответ сервера, добавив туда время мерджа. Знаю, что сделал позже дедлайна, но я решил, что лучше поздно, чем никогда.
...
```
## Линтер
Я взял golden Golden config for golangci-lint, т.к. это лучший выбор для проверки формата кода